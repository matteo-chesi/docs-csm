...
1. Upload the `data.json` file to BSS, our `cloud-init` data source.
...
1. Preserve logs and configuration files if desired.
...
1. Backup the bootstrap information from `ncn-m001`.
...
1. Wait for the node to boot, acquire its hostname (`ncn-m001`), and run `cloud-init`.
    ...
    (`ncn-m001#`)
    ```bash
    cloud-init clean ; cloud-init init ; cloud-init modules -m init ; \
...
1. Verify `zypper` repositories are empty and all remote SUSE repositories are disabled.
    ...
    (`ncn-m001#`)
    ```bash
    rm -v /etc/zypp/repos.d/* && zypper ms --remote --disable
    ```
...
1. Set `BMCS` variable to list of the BMCs for all master, worker, and storage nodes,
    ...
    (`ncn-m001#`)
    ```bash
    BMCS=$(grep -Eo "[[:space:]]ncn-[msw][0-9][0-9][0-9]-mgmt([.]|[[:space:]]|$)" /etc/hosts |
...
1. (`ncn-m001#`) Run the following to loop through all of the BMCs (except `ncn-m001-mgmt`) and apply the desired settings.
    ...
    - Get the NMN DNS IP value
        ...
        ```bash
        export NMN_DNS=$(kubectl get services -n services -o wide | grep cray-dns-unbound-udp-nmn | awk '{ print $4 }')
        ```

    - Get the HMN DNS IP value

        ```bash
        export HMN_DNS=$(kubectl get services -n services -o wide | grep cray-dns-unbound-udp-hmn | awk '{ print $4 }')
        ```
